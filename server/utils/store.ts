...
export async function upsertTodo(
    todo: Todo,
): Promise<{ todo: Todo; wasUpdated: boolean }> {
 ...
 * Optionally exclude a set of IDs (to avoid echoing back client changes)
 */
export async function getTodosModifiedAfter(
    userId: string,
    timestamp: string,
    excludeIds?: string[],
): Promise<Todo[]> {
        ...
        .where(
            and(
                ...
                excludeIds && excludeIds.length > 0
                    ? sql`${todosTable.id} NOT IN (${sql.join(
 ...
 * Fetches all existing records in one query, then uses ON CONFLICT upsert
 */
export async function bulkUpsertTodos(todosToUpsert: Todo[]): Promise<{
    updated: Todo[];
    conflicts: Todo[];
}> {
    if (todosToUpsert.length === 0) return { updated: [], conflicts: [] };

    // Fetch all existing records in one query instead of N individual SELECTs
    const ids = todosToUpsert.map((t) => t.id);
    ...
    for (const todo of todosToUpsert) {
        ...
        if (existing) {
            ...
            const existingTime = new Date(existing.updatedAt).getTime();
            if (existingTime > incomingTime) {
                conflicts.push(toTodo(existing));
                continue;
            }
        }
        toWrite.push(todo);
        updated.push({ ...todo, syncStatus: "synced" });
    }

    // Batch write all winning todos using ON CONFLICT upsert
    for (const todo of toWrite) {
        await db
            .insert(todosTable)
            .values({
                id: todo.id,
            ...
            .onConflictDoUpdate({
                ...
                set: {
                    ...
                    rolloverRule: sql`excluded.rollover_rule`,
                    rolloverFromId: sql`excluded.rollover_from_id`,
