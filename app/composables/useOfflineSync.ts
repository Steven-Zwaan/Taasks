...
export function useOfflineSync() {
  ...
  const temporaryBannerState = ref<'syncing' | 'offline' | 'online' | 'error' | null>(null)
  ...
  if (import.meta.client) {
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'visible' && isOnline.value) {
        ...
        if (timeSinceLastSync > VISIBILITY_SYNC_COOLDOWN && !syncInProgress) {
          syncImmediateThrottled()
        }
      }
    })
   ...
   */
  function showTemporaryNotification(state: 'syncing' | 'offline' | 'online' | 'error'): void {
  ...
  async function updatePendingCount(): Promise<void> {
    ...
    if (pendingCount.value > 0 && isOnline.value && syncState.value === 'idle') {
  ...
  async function syncImmediate(): Promise<boolean> {
    ...
    try {
      ...
      const { serverChanges, conflicts = [], syncTimestamp } = response as {
        serverChanges: Todo[]
      ...
      // Track if server had newer data
      let serverHadNewerData = conflicts.length > 0
        ...
        for (const serverTodo of serverChanges) {
          ...
          if (!localTodo) {
            ...
            if (serverTime > localTime) {
              ...
              serverHadNewerData = true
            }
            // Otherwise keep local version (will sync on next request if needed)
          }
        }

        // Process conflicts - these are cases where server had newer data
        // We need to update local with server version
